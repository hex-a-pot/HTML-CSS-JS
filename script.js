...
for(let i = 0; i<365; i++)
{
    ...
    elem.className = list.includes(i) ? "box active" : "box"
...
window.addEventListener("scroll",()=>{
    ...
    jobTitles[1].style.transform = `translateX(calc(${offSetY}px - 300vh))`
    projectTitles.style.transform = `translateY(calc(400vh - ${offSetY}px))`
    phone.style.transform = `translateX(calc(500vh - ${offSetY}px))`
    laptop.style.transform = `translateX(calc(${offSetY}px - 600vh))`
})